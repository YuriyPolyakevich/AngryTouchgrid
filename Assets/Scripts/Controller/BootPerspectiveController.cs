...
{
    public class BootPerspectiveController : MonoBehaviour
    {
        ...
        private void GetSlingShotController()
        {
            ...
            slingShotController.BootPerspectiveController = GetComponent<BootPerspectiveController>();
            slingShotController.Boot = gameObject;
        }

        private void GetBootController()
        {
            if (GetComponent<BootPerspectiveController>() == null)
            {
        ...
        Ray GenerateTouchRay(Touch touch)
        {
        ...
        private void StartDraggingConfiguration(Touch touch)
        {
            ...
            generatedPlane = new Plane(Camera.main.transform.forward * -1, transform.position);

            Ray tRay = Camera.main.ScreenPointToRay(touch.position);
            float rayDistance;
            generatedPlane.Raycast(tRay, out rayDistance);
            touchOffset = transform.position - tRay.GetPoint(rayDistance);
        ...
        {
            Ray tRay = Camera.main.ScreenPointToRay(touchPosition);
            float rayDistance;
            if (generatedPlane.Raycast(tRay, out rayDistance))
            {
                var xPosition = Mathf.Clamp(tRay.GetPoint(rayDistance).x + touchOffset.x, _initialPosition.x - BootConstantsUtil.XDistanceConstraint,
                _initialPosition.x + BootConstantsUtil.XDistanceConstraint);
                var yPosition = Mathf.Clamp(tRay.GetPoint(rayDistance).y + touchOffset.y, _initialPosition.y - BootConstantsUtil.YDownDistanceConstraint,
